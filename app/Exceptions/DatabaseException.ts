	...
	public async handle(error: this, { response, logger }: HttpContextContract) {
		switch (error.code) {
			case '23505':
				...
				return response.status(500).send({ status: false, error:  'Data already exist in database'})
			default:
				return response.status(500).send({ status: false, error: 'Unknow error in database' })
		}
	}
}
